...
# endpoint (see Get-CBGraphToken / Get-CBStorageToken in the Collaborate module).
#
...
# never auto-imports it and every function fails with 'Initialize-CBTables is
# not recognized'. Importing by path here is deterministic on every worker.
Import-Module (Join-Path $PSScriptRoot 'Modules/Collaborate/Collaborate.psd1') -Force -ErrorAction Stop
